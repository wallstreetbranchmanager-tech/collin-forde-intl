...
const LEADS_EMAIL = process.env.LEADS_EMAIL || "collin.forde.international@gmail.com";
...
function formatBooking(data: Booking) {
  return [
    ...
    `Phone: ${data.phone}`,
    `Date: ${data.date}`,
    `Time: ${data.time} America/New_York`,
...
async function sendViaFormSubmit(subject: string, payload: Record<string, string>) {
  const res = await fetch(`https://formsubmit.co/ajax/${encodeURIComponent(LEADS_EMAIL)}`, {
    method: "POST",
...
export async function deliverInquiry(data: Inquiry) {
  const subject = `[${data.type.toUpperCase()}] ${data.name} — Collin Forde Intl`;
  const text = formatInquiry(data);
  const sentResend = await sendViaResend(subject, text, data.email);
  if (sentResend) return { channel: "resend" as const };
  ...
  if (sentForm) return { channel: "formsubmit" as const };
...
export async function deliverBooking(data: Booking) {
  const subject = `VIEWING ${data.date} ${data.time} — ${data.name}`;
